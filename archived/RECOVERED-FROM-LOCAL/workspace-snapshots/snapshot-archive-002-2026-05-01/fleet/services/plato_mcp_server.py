...
ATOM_TOOL = {
    "name": "plato_atom",
    "description": (
        "Submit a reasoning atom to an active decomposition session. "
        "Chain: premise→reasoning→hypothesis→verification→conclusion. "
        "Each atom becomes a PLATO tile with dependencies.\n\n"
        "Typical 3-call pattern:\n"
        "1. plato_atom(room=..., atom_id='P1', content='API returns 500', atom_type='premise')\n"
        "2. plato_atom(room=..., atom_id='R1', content='Likely DB timeout', atom_type='reasoning', depends_on=['P1'])\n"
        "3. plato_atom(room=..., atom_id='C1', content='Add connection pooling', atom_type='conclusion', depends_on=['R1'], confidence=0.95, is_verified=true)"
    ),
    "inputSchema": {
        "type": "object",
        "properties": {
...
DECOMPOSE_ACTION_TOOL = {
    "name": "plato_decompose_action",
    "description": "Perform decomposition lifecycle actions: start decomposing an atom into sub-atoms, or contract verified sub-atoms back to their parent.",
    "inputSchema": {
        "type": "object",
        "properties": {
            "action": {"type": "string", "enum": ["decompose", "contract"], "description": "Action to perform"},
            "room": {"type": "string", "description": "Decomposition room name"},
            "atom_id": {"type": "string", "description": "Atom ID to decompose (required for decompose)"},
            "decomposition_id": {"type": "string", "description": "Decomposition ID to contract (required for contract)"}
        },
...
def get_all_tools() -> List[Dict[str, Any]]:
    """Return base tools + decomposition tools."""
    return TOOLS + [DECOMPOSE_TOOL, ATOM_TOOL, REASONING_STATUS_TOOL, DECOMPOSE_ACTION_TOOL]


def api_get(url: str, timeout: int = DEFAULT_TIMEOUT) -> Dict[str, Any]:
    """GET from a fleet service."""
    try:
        req = urllib.request.Request(url, headers={"User-Agent": f"plato-mcp/{MCP_VERSION}"})
...
def api_post(url: str, data: Dict[str, Any], timeout: int = DEFAULT_TIMEOUT) -> Dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            url,
            data=body,
            headers={"Content-Type": "application/json", "User-Agent": f"plato-mcp/{MCP_VERSION}"},
            method="POST",
        )
        with urllib.request.urlopen(req, timeout=timeout) as r:
            return json.loads(r.read())
...
def _text_response(data: Dict[str, Any], is_error: bool = False) -> Dict[str, Any]:
    """Build a standard MCP text content response."""
    return {"content": [{"type": "text", "text": json.dumps(data, indent=2)}], "isError": is_error}
...
def handle_tool(name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name == "plato_search":
        ...
        if domain:
            ...
            result = api_get(f"{PLATO_URL}/search?q={quote(query)}")
        tiles = result.get("tiles", result.get("results", []))
        summaries = [
            {"q": t.get("question", "")[:100], "a": t.get("answer", "")[:200], "confidence": t.get("confidence", 0)}
            for t in tiles[:limit]
        ]
        return _text_response({"query": query, "total": len(tiles), "results": summaries})

    elif name == "plato_submit":
        result = api_post(f"{PLATO_URL}/submit", args)
        if "error" in result:
            return _error_response(result["error"])
        return _text_response({"status": "accepted", "tile_hash": result.get("tile_hash", "")})
        ...
        rooms = [
            {"name": k, "tiles": v.get("tile_count", 0)}
            for k, v in result.items()
            if v.get("tile_count", 0) >= min_tiles
        ] if isinstance(result, dict) else []
        rooms.sort(key=lambda x: -x["tiles"])
        return _text_response({"total": len(rooms), "rooms": rooms[:50]})
        ...
        if action == "leaderboard":
            ...
            lb = [
                {
                    "agent": p.get("agent", ""),
                    "rating": round(p.get("rating", p.get("mu", 0))),
                    "w": p.get("wins", 0),
                    "l": p.get("losses", 0),
                }
                for p in result.get("leaderboard", [])
            ]
            return _text_response({"leaderboard": lb})
        elif action == "register":
            result = api_get(f"{ARENA_URL}/register?agent={args.get('agent', '')}")
            return _text_response(result)
        elif action == "match":
            result = api_get(
                f"{ARENA_URL}/match?player_a={args.get('agent', '')}&player_b={args.get('opponent', '')}"
                f"&game={args.get('game', 'reasoning')}&winner={args.get('winner', 'draw')}"
            )
            return _text_response(result)
        return _text_response({"error": f"Unknown arena action: {action}"}, is_error=True)

    elif name == "plato_validate":
        assertions = args.get("assertions", [])
        result = api_post(
            f"{VALIDATION_URL}/validate",
            {"domain": "mcp", "question": "validation", "answer": "test", "assertions": assertions},
        )
        return _text_response(result)
        ...
        return _text_response({
            "plato": {"version": plato.get("version", "?")},
            "mud": {"rooms": mud.get("rooms", 0), "agents": mud.get("agents", 0)},
        })
        ...
        room = args.get("room", "")
        if not room:
            return _error_response("room required")
        atom = {
    ...
    elif name == "plato_decompose_action":
        ...
        if action == "decompose":
            atom_id = args.get("atom_id", "")
            if not atom_id:
                return _error_response("atom_id required for decompose")
            result = api_post(f"{PLATO_URL}/decompose/{room}/decompose-atom", {"atom_id": atom_id})
        elif action == "contract":
            decomp_id = args.get("decomposition_id", "")
            if not decomp_id:
                return _error_response("decomposition_id required for contract")
            result = api_post(f"{PLATO_URL}/decompose/{room}/contract", {"decomposition_id": decomp_id})
        else:
...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        """Handle GET requests for MCP discovery and health."""
        ...
        if parsed.path == "/":
            ...
            self._json({"status": "alive", "tools": len(get_all_tools()), "version": MCP_VERSION})
        else:
...
def stdio_mode() -> None:
    ...
    print(json.dumps({"jsonrpc": "2.0", "method": "initialize", "params": {
        ...
        "serverInfo": {"name": "plato-mcp", "version": MCP_VERSION}
    }}), flush=True)
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if method == "tools/list":
                response = {"jsonrpc": "2.0", "id": msg_id, "result": {"tools": get_all_tools()}}
...
if __name__ == "__main__":
    ...
    if args.stdio:
        ...
        print(f"🔮 PLATO MCP Server v{MCP_VERSION} on port {args.port}")
        print(f"   {len(all_tools)} tools: {', '.join(t['name'] for t in all_tools)}")
